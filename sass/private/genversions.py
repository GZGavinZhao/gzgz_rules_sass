...
for version, assets in filter(lambda tup: tup[0] not in versions.SASS_VERSIONS, data.items()):
    ...
    for k, v in assets.items():
        platform = name_to_platform(k)
        if not platform:
            continue

        if platform in res[version] and "musl" not in k:
            print(f"Skipping {k} in v{version} because a musl version exists")
            continue
        res[version][platform] = calculate_sha384_base64(v)
